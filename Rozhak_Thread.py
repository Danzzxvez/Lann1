...
[bold white]     |  |   |  | |  ||  |   \   --.\ '-'  |\ `-' |.-'  `) 
[bold white]     `--'   `--' `--'`--'    `----' `--`--' `---' `----'  
     [bold white]Version :[bold blue] 5.0[bold white] || Author :[bold green] Moch Nangg[bold white] || Type :[bold blue] Hacking"""))
        return ("Sukses")
    ...
    def Terminal_Size(self):
        ...
        if int(self.get_terminal_size) < 65:
            ...
            if 'TERMUX_VERSION' in str(os.environ) or 'nangg' in str(os.environ) or 'LAPTOP-LMA1HAAJ' in str(os.environ):
                return ("Sukses")
            else:
                self.Clear()
